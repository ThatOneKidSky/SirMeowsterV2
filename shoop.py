...
class shoop():
    """A shoop instance where a user can spend points."""
    ...
    #region Page updates
    async def open(self):
        """Opens the current page. (no default)"""
    ...
    def handleButtonPrice(self, item):
        """Takes in button data and determines the price accordingly."""
        label = ""
    ...
    async def backArrow(self, interaction, page):
        """Uses the page's back arrow data to go back a page."""
        await self.goto(shoopPages[page]["backArrow"])
        self.lock = False

    async def onClick(self, bot, interaction, page, buttonId):
        """Handles the event where a user presses a button."""
    ...
    async def horseRoll(self, bot, interaction, buttonData):
        """The buy function for a horse roll, specified in shoop/pages.json."""
        try:
    ...
    async def closeButton(self, bot, interaction, buttonData):
        """The button to close a shoop."""
        await self.close()

    async def close(self):
        """Closes the shoop instance."""
        if self.message:
            try:
