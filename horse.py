import random, discord, utilities, asyncio, math
import discord.types
from variables import playerData, horseData, constants
from utilities import safeFormat, log

#region Main
async def roll(userId: str, bot: discord.Client, aDiv = 0):
    ...
    try:
        ...
        if choice < winChance: # If the user wins
            await rollWin(userId, bot, uDiv + Div + aDiv)
            return True
...
#region Utils
class rate:
    def __init__(self, val, mean, volatility = 1, shockChance = 0.01, shockWeight = 5):
        self.mean = mean
        self.val = val
        self.trend = 0
        self.volatility = volatility
        self.shockChance = shockChance
        self.shockWeight = shockWeight
    
    def step(self):
        self.trend += random.gauss(0, 1)
        self.trend = self.trend / 2

        shock = random.gauss(0, self.shockWeight) if random.random() < self.shockChance else 0

        distance = self.val - self.mean
        reversion = -distance * math.exp(min(abs(distance) * 0.01,10))

        noise = random.gauss(0, self.volatility)

        self.val = self.val + noise + shock + reversion

RNG = horseData["RNG"]
Win  = rate(RNG["win"],  RNG["winm"],  volatility=2,   shockChance=0.1)
Loss = rate(RNG["loss"], RNG["lossm"], volatility=1,   shockChance=0.01)
Lost = rate(RNG["lost"], RNG["lostm"], volatility=0.6, shockChance=0.01, shockWeight=10)
Div  = RNG["divisor"]

def stepRNG():
    global Win, Loss, Lost
    try:
        Win.step()
        Loss.step()
        Lost.step()

        horseData["RNG"]["win"] = Win.val
        horseData["RNG"]["loss"] = Loss.val
        horseData["RNG"]["lost"] = Lost.val

        log(f"New rng values: {Win.val} - {Loss.val} - {Lost.val}", "Data")
        
        horseData.save()
        return True
    except Exception as e:
        print(e)
        print(f"ERROR in stepRNG: {e}")

def getRNG():
    """Returns the W/L/L/D as set in data/horse.json."""
    try:
        return max(int(Win.val), 1), max(int(Loss.val), 1), max(int(Lost.val), 1), float(Div)
    except Exception as e:
...
async def rollWin(userId: str, bot: discord.Client, div: float):
    ...
    try:
        divisor = div if div >= 0 else div * 2
...
async def rollLost(userId: str, bot: discord.Client):
    ...
    try:
        ...
        roll = random.choice(horseData["lost"])
